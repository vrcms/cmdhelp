...
### 基本用法
给出该命令最精简可运行的典型调用，格式为 \`命令 参数\`，并用一句话说明。

### 常用选项
按功能分类整理最常用或最重要的参数，使用 Markdown 表格呈现，表头为 | 参数 | 说明 |，每行一个参数。覆盖足够信息，让用户看完基本能直接上手，不要只挑最简的几个。
示例表格行：| \` -p <端口>\` | 指定远程主机端口，默认 22，如 \`-p 2222\` |
若某类参数较多，可按“连接 / 认证 / 转发 / 调试”等分类分多张表。
